    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <span style={{ color: "var(--text-main)", fontSize: 12, fontWeight: 700 }}>@{username}</span>
        </div>
        <a
          href={`https://github.com/${username}`}
          target="_blank"
          rel="noopener noreferrer"
          style={{ color: "var(--text-muted)", fontSize: 11, fontWeight: 600, textDecoration: "none", display: "flex", alignItems: "center", gap: 4 }}
        >
          View Profile
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8, marginBottom: 16 }}>
        {stats.map((s) => (
          <div key={s.label} style={{ textAlign: "center", padding: "10px 8px", borderRadius: 10, background: "var(--card-bg)", border: "1px solid var(--card-border)" }}>
            <div style={{ fontSize: 16, marginBottom: 4 }}>{s.icon}</div>
            <div style={{ color: "var(--text-main)", fontSize: 15, fontWeight: 800, lineHeight: 1 }}>{loading ? "—" : s.value}</div>
            <div style={{ color: "var(--text-muted)", fontSize: 10, fontWeight: 600, marginTop: 3, textTransform: "uppercase", letterSpacing: "0.08em" }}>{s.label}</div>
          </div>
        ))}
      </div>
      ...
      {/* Contribution grid */}
      <div>
        <div style={{ color: "var(--text-muted)", fontSize: 10, fontWeight: 600, marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.1em" }}>
          Contribution Activity
        </div>
        {loading ? (
          <div style={{ height: 80, borderRadius: 8, background: "var(--card-bg)", display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 4, marginTop: 8, justifyContent: "flex-end" }}>
          ...
          <span style={{ color: "var(--text-muted)", fontSize: 10 }}>More</span>
        </div>
      </div>
    </motion.div>
  );
