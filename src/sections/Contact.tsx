...
const contactInfo = [
  ...
  { label: "Phone", value: "+91 9211942792", href: "tel:+919211942792", color: "#34d399", icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12 19.79 19.79 0 0 1 1.61 3.4 2 2 0 0 1 3.6 1.22h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 8.8a16 16 0 0 0 6.29 6.29l.95-.95a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z" /></svg> },
...
const inputStyle: React.CSSProperties = {
  ...
  color: "#e2e8f0", fontSize: 14, outline: "none", transition: "all 0.2s", fontFamily: "inherit",
};
  ...
  return (
    <SectionWrapper id="contact" title="Get In Touch" subtitle="Contact">
      <motion.div
        ...
        style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1.3fr", gap: isMobile ? 32 : 48, alignItems: "start" }}
      >
        {/* Left */}
        <div>
          <h3 style={{ color: "#fff", fontWeight: 700, fontSize: isMobile ? 18 : 22, marginBottom: 10 }}>Let&apos;s build something together</h3>
            ...
            {contactInfo.map((item) => (
              ...
              >
                ...
                <div>
                  ...
                  <div style={{ color: "#cbd5e1", fontSize: 13, fontWeight: 600 }}>{item.value}</div>
                </div>
              </a>
            ))}
          </div>
        ...
        <form onSubmit={handleSubmit}
          style={{ padding: isMobile ? "24px 20px" : "32px", borderRadius: 20, border: "1px solid rgba(255,255,255,0.07)", background: "rgba(255,255,255,0.02)", backdropFilter: "blur(8px)", display: "flex", flexDirection: "column", gap: 14, position: "relative", overflow: "hidden" }}
        >
          ...
          <div>
            <input type="text" placeholder="Your name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} style={{ ...inputStyle, borderColor: errors.name ? "rgba(239,68,68,0.5)" : "rgba(255,255,255,0.08)" }} />
            {errors.name && <p style={{ color: "#f87171", fontSize: 12, marginTop: 6 }}>{errors.name}</p>}
          </div>
          <div>
            <input type="email" placeholder="your@email.com" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} style={{ ...inputStyle, borderColor: errors.email ? "rgba(239,68,68,0.5)" : "rgba(255,255,255,0.08)" }} />
            {errors.email && <p style={{ color: "#f87171", fontSize: 12, marginTop: 6 }}>{errors.email}</p>}
          </div>
          <div>
            <textarea placeholder="Tell me about your project or opportunity..." rows={5} value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} style={{ ...inputStyle, resize: "none", borderColor: errors.message ? "rgba(239,68,68,0.5)" : "rgba(255,255,255,0.08)" }} />
            {errors.message && <p style={{ color: "#f87171", fontSize: 12, marginTop: 6 }}>{errors.message}</p>}
          </div>
